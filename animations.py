...
class animations:
    ...
    def LCS_and_particle_advection(self, start_time, outfile, LCS_start_time=0):
        ...
        def par(i):
            ...
            file = ip.displacement_aggregated(start_time, at_time=i, outfile=i, par=True)
            LCS = FTLE(file)
            return LCS['ALCS']
        ...
        def animation(i):
            ...
            ax.pcolormesh(in_x, in_y, x, cmpa='jet', vmin=-0.2, vmax=0.5, transform=plate)
    ...
    def LCS_and_particle_advection_gyre(self, t0, t1, outfile, epsilon=0.25, omega=0.682, A=0.1):
        ...
        def par(i):
            file = edg(at_time=self.times[i], dur=self.dur, time_step=self.ts, sep=self.sep, epsilon=epsilon, omega=omega, A=A, forwards=False, outfile=i)
            LCS = FTLE(file)
            return LCS['ALCS']
...
if __name__ == '__main__':
    ...
    duration = 3600
    dxdy = 0.02
    start_time=[2022,5,21,0] #no data before 2022 5 21 00
    # start time particle advection 2022 5 20 12 - 2022 5 30 12
